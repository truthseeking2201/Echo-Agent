...
export const LayoutShell: React.FC<LayoutShellProps> = ({ 
  ...
  const { wallet, connectWallet, disconnectWallet, settings, togglePause } = useEchoStore();
  ...
  const handleWalletAction = async () => {
    if (wallet.connected) {
      ...
      try {
        ...
        showToast(`Wallet connected: ${address.substring(0,6)}...${address.substring(address.length-4)}`, "success");
      } catch (error) {
        ...
        <div className="max-w-content px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <motion.div
              ...
              className={`
                flex items-center space-x-2 px-3 py-1.5 rounded-full text-sm transition-colors duration-fast cursor-pointer
                ${wallet.connected ? 'bg-panel hover:bg-white/10' : 'bg-primary/20 hover:bg-primary/30'}
                ${isConnecting ? 'opacity-70 cursor-default' : ''}
              `}
              onClick={isConnecting ? undefined : handleWalletAction}
            >
              <div className={`h-2 w-2 rounded-full ${wallet.connected ? 'bg-success' : 'bg-error'} ${isConnecting ? 'animate-pulse': ''}`}></div>
              <span className="font-mono">
                {isConnecting ? 'Connecting...' : wallet.connected 
                  ? `${wallet.addr?.substring(0, 6)}...${wallet.addr?.substring(wallet.addr.length - 4)}` 
                  : 'Connect Wallet'
                }
              </span>
            </motion.div>
            ...
            {/* Settings button */}
            <Link href="/settings">
              <a className="flex items-center justify-center w-8 h-8 rounded-full bg-panel text-white/70 hover:text-white hover:bg-white/10 transition-colors duration-fast">
                {settingsIcon}
              </a>
            </Link>
            ...
            <li>
              <Link href="/settings">
                <a 
                  className={`
                    flex flex-col items-center justify-center px-4 py-2
                    ${router.pathname === '/settings' ? 'text-primary' : 'text-white/60'}
                  `}
                >
                  <div className="w-6 h-6 mb-1">
                    {settingsIcon}
      ...
      `}>
        <div className="max-w-content mx-auto">
          <motion.div
            ...
            transition={{ duration: durations.slow, ease: standardEasing }}
            className="relative"
          >
            {title && (
              <motion.h1 
